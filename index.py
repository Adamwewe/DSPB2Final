...
from apps import homepage, overview, bridge_selector, about, contact, data
...
engine = create_engine(
    'postgresql://kcplaimkickuot:956b869a2ba2920cd92abdaa70561d9bab6553175a5d83603e7c3cf89991803f@ec2-52-31-217-108.eu-west-1.compute.amazonaws.com:5432/d1rk75lc30behl')
...
# credits login: https://github.com/naderelshehabi/dash-flask-login
                ...
                dbc.Nav([
                    dbc.DropdownMenu(
                        children=[
                            ...
                            dbc.DropdownMenuItem("Data Collaboration", href="/apps/data"),
                        ],
                        nav=True,
                        in_navbar=True,
                        label="The System",
...
def login_button_click(n_clicks, username, password):
    if n_clicks > 0:
        ...
        # retrieve all users
        sql_query = pd.read_sql('SELECT username, password FROM "USERS"', session.bind)

        users = dict()
        for index, row in sql_query.iterrows():
            users[row['username']] = row['password']

        # check if hashed password matches the user input
